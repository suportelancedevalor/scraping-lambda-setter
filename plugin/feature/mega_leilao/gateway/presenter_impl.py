...
from features.mega_leilao.domain.mega_leilao_house import House, HousesToReturn
...
class PresenterImpl(GatewayInjector, BaseController, Presenter):
    ...
    def handle_success(self, value):
        if isinstance(value, HousesToReturn):
            return self.on_sequence(value.houses)

    def on_sequence(self, houses: List[House]):
